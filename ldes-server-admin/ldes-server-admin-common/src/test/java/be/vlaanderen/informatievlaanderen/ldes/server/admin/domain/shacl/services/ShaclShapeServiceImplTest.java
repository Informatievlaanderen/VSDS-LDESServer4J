...
class ShaclShapeServiceImplTest {
	...
	class RetrieveShaclShape {
		...
		void when_collectionExists_and_hasShapeConfigured_then_retrieveShape() throws URISyntaxException {
			...
			Mockito.when(shaclShapeRepository.retrieveShaclShape(COLLECTION_NAME_1))
			...
			InOrder inOrder = Mockito.inOrder(shaclShapeRepository, eventPublisher);
		...
		void when_collectionDoesNotExists_then_throwException() {
			Mockito.when(shaclShapeRepository.retrieveShaclShape(COLLECTION_NAME_1)).thenReturn(Optional.empty());
	...
	class UpdateShaclShape {
		...
		void when_collectionExists_and_updateShape_then_expectUpdatedShacl() throws URISyntaxException {
			...
			inOrder.verify(eventPublisher).publishEvent(ArgumentMatchers.any(ShaclChangedEvent.class));
			inOrder.verifyNoMoreInteractions();
		}
	}
	...
	class DeleteShaclShape {
		...
		void when_collectionExists_and_deleteShape_then_throwExceptionWithRetrieval() {
			...
			inOrder.verify(eventPublisher).publishEvent(ArgumentMatchers.any(ShaclDeletedEvent.class));
			inOrder.verifyNoMoreInteractions();
		}
	}
	...
	class InitShapes {
		...
		void when_ApplicationIsStarted_ShaclChangedEventsAreSentOut() {
			...
			Mockito.when(shaclShapeRepository.retrieveAllShaclShapes()).thenReturn(List.of(shaclShape, shaclShape2));

			service.initShapes();

			InOrder inOrder = Mockito.inOrder(shaclShapeRepository, eventPublisher);
			inOrder.verify(shaclShapeRepository).retrieveAllShaclShapes();
			inOrder.verify(eventPublisher, Mockito.times(2)).publishEvent((ShaclChangedEvent) ArgumentMatchers.any());
			inOrder.verifyNoMoreInteractions();
		}
	}
