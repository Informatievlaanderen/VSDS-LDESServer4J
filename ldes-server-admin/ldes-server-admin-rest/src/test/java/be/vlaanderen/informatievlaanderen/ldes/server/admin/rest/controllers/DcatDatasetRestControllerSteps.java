...
public class DcatDatasetRestControllerSteps extends SpringIntegrationTest {
	...
	public void theMetadataWillBeStored() {
		Model model = RDFParser.create().fromString(datasetString).lang(Lang.TURTLE).build().toModel();
	...
	public void theDataserviceDoesNotYetExist() {
		when(dcatDatasetRepository.exitsByCollectionName(COLLECTION_NAME)).thenReturn(false);
	}
