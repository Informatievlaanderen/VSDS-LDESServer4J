...
class ConnectedFragmentsFinderTest {
	...
	ConnectedFragmentsFinder connectedFragmentsFinder = new ConnectedFragmentsFinder(ldesFragmentRepository,
			relationsAttributer, fragmentGenerator);
	...
	void when_OtherFragmentsExist_MethodReturnsAllConnectedFragmentsToClosestFragment() {
		...
		when(fragmentGenerator.generateFragmentPathToClosestFragment(ldesFragment, availableFragments))
				.thenReturn(List.of(ldesFragment, getLdesFragment(connectedFragmentids.get(0)),
						getLdesFragment(connectedFragmentids.get(1)), availableFragments.get(0)));

		List<LdesFragment> connectedFragments = connectedFragmentsFinder.findConnectedFragments(ldesFragment);

		assertEquals(4, connectedFragments.size());
		assertEquals(Set.of(FRAGMENT_ID, connectedFragmentids.get(0), connectedFragmentids.get(1), closestFragmentId),
