...
import be.vlaanderen.informatievlaanderen.ldes.fragmentisers.geospatial.connected.relations.GeospatialRelationsAttributer;
...
class ConnectedFragmentsFinderTest {
    ...
    private final FragmentGenerator fragmentGenerator = mock(FragmentGenerator.class);

    ConnectedFragmentsFinder connectedFragmentsFinder = new ConnectedFragmentsFinder(ldesFragmentRepository, relationsAttributer, fragmentGenerator);
    ...
    @Test
    @DisplayName("ConnectedFragmentsFinder returns all connected fragments with updated relations")
    void when_OtherFragmentsExist_MethodReturnsAllConnectedFragmentsToClosestFragment() {
        LdesFragment ldesFragment = getLdesFragment(FRAGMENT_ID);
        ...
        List<LdesFragment> availableFragments = availableFragmentsIds.stream().map(this::getLdesFragment).toList();
        when(ldesFragmentRepository.retrieveAllFragments()).thenReturn(availableFragments);
        when(fragmentGenerator.generateFragmentPathToClosestFragment(ldesFragment, availableFragments)).thenReturn(List.of(ldesFragment, getLdesFragment(connectedFragmentids.get(0)), getLdesFragment(connectedFragmentids.get(1)), availableFragments.get(0)));

        List<LdesFragment> connectedFragments = connectedFragmentsFinder.findConnectedFragments(ldesFragment);

        assertEquals(4, connectedFragments.size());
        assertEquals(Set.of(FRAGMENT_ID, connectedFragmentids.get(0), connectedFragmentids.get(1), closestFragmentId), connectedFragments.stream().map(LdesFragment::getFragmentId).collect(Collectors.toSet()));
        assertTrue(connectedFragments.stream().allMatch(ldesFragment1 -> ldesFragment1.getRelations().size() >= 1));
    }
