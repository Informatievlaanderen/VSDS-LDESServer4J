...
class TreeMemberRemoverImplTest {
	...
	void when_memberHasNoReferences_ItCanBeDeleted() {
		when(memberRepository.getMember("memberId")).thenReturn(Optional.of(new Member("memberId", "collection", 0L, null, null, null, List.of())));
	...
	void when_memberHasReferences_ItCannotBeDeleted() {
		when(memberRepository.getMember("memberId"))
				.thenReturn(Optional.of(new Member("memberId", "collection", 0L, null, null, null, List.of("reference"))));
		treeMemberRemover.tryRemovingMember("memberId");
