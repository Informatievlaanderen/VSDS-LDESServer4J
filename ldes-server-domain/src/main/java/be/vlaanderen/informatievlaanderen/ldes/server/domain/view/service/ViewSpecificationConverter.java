...
public class ViewSpecificationConverter {
    ...
    private List<Statement> fragmentationStatementsFromList(Resource viewName,
        ...
        List<ResourceImpl> fragmentationResources = fragmentationList.stream().map(fragmentation -> {
            ...
            resource.addLiteral(createProperty(CUSTOM + FRAGMENTATION_NAME), createPlainLiteral(fragmentation.getName()));
            fragmentation.getConfig().forEach((key, value) -> resource.addProperty(createProperty(CUSTOM + key), createPlainLiteral(value)));
            return resource;
        }).toList();
