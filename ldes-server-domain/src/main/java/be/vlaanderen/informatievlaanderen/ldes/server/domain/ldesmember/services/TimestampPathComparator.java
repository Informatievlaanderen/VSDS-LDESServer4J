...
public class TimestampPathComparator implements Comparator<LdesMember> {
	...
	public int compare(LdesMember firstLdesMember, LdesMember secondLdesMember) {
		LocalDateTime dateTimeFirstLdesMember = LocalDateTime
				.parse(firstLdesMember.getFragmentationObject(PROV_GENERATED_AT_TIME).toString(), formatter);
		LocalDateTime dateTimeSecondLdesMember = LocalDateTime
				.parse(secondLdesMember.getFragmentationObject(PROV_GENERATED_AT_TIME).toString(), formatter);
