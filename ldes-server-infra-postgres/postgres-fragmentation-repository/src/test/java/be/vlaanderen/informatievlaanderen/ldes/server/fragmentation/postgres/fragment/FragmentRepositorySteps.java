...
public class FragmentRepositorySteps extends PostgresFragmentationIntegrationTest {
		...
		return new FragmentWithRelation(new Fragment(new LdesFragmentIdentifier(
                ...
                row.get("relations").isEmpty() ? List.of()
						: Arrays.stream(row.get("relations").split(",")).map(treeNode -> new TreeRelation("",
								LdesFragmentIdentifier.fromFragmentId(treeNode), "", "", RdfConstants.GENERIC_TREE_RELATION))
								.toList(),
				null));
	}
	...
	public void theRetrievedLdesFragmentHasTheSamePropertiesAsLdesFragment(int index) {
		Assertions.assertTrue(retrievedLdesFragment.isPresent());
		Fragment actualFragment = this.retrievedLdesFragment.get();
		Fragment expectedFragment = this.fragments.get(index - 1);
		Assertions.assertEquals(expectedFragment.getFragmentId(), actualFragment.getFragmentId());
		Assertions.assertEquals(expectedFragment.getViewName(), actualFragment.getViewName());
		Assertions.assertEquals(expectedFragment.getFragmentPairs(), actualFragment.getFragmentPairs());
		Assertions.assertEquals(expectedFragment.isImmutable(), actualFragment.isImmutable());
		Assertions.assertEquals(expectedFragment.getNrOfMembersAdded(), actualFragment.getNrOfMembersAdded());
		Assertions.assertEquals(expectedFragment.getRelations(), actualFragment.getRelations());
	}
	...
	public void theRetrievedLdesFragmentHasGivenNrOfMembersAdded(int nrOfMembersAdded) {
		Assertions.assertTrue(retrievedLdesFragment.isPresent());
		Fragment actualFragment = this.retrievedLdesFragment.get();
		Assertions.assertEquals(nrOfMembersAdded, actualFragment.getNrOfMembersAdded());
	}
	...
	public void theRepositoryTheFollowingFragmentsWithOutgoingRelations(
		...
		outgoingRelationResults.forEach(outgoingRelationResult -> {
			...
			Assertions.assertEquals(outgoingRelationResult.fragmentIds,
	...
	public void theRepositoryHasTheFollowingFragmentsLeft(List<FragmentWithRelation> fragmentWithRelations) {
		fragmentWithRelations.forEach(fragmentWithRelation -> {
			...
			Assertions.assertEquals(fragmentWithRelation.fragment.getRelations()
					.stream()
