...
public interface CompactionPageEntityRepository extends JpaRepository<CompactionPageEntity, Long> {
	...
	@Query(value = """
			SELECT p.page_id                                                     as fragmentId,
			       (SELECT COUNT(*) FROM page_members WHERE page_id = p.page_id) AS size,
			       r.to_page_id                                                  AS toPage,
			       p.bucket_id                                                   AS bucketId,
			       p.partial_url                                                 AS partialUrl
			FROM pages p
			...
			WHERE c.name = :collectionName
			  ...
			  AND p.immutable
			  AND NOT p.is_root
			  AND (SELECT COUNT(*) FROM page_members WHERE page_id = p.page_id) < :capacityPerPage
