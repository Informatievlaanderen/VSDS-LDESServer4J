...
import be.vlaanderen.informatievlaanderen.ldes.server.fetch.postgres.mapper.FetchMemberModelConverter;
...
public class FetchMemberEntity {
    @Id
    ...
    @Convert(converter = FetchMemberModelConverter.class)
    @Column(name = "member_model", nullable = false, columnDefinition = "bytea")
    private Model model;

	@SuppressWarnings("java:S107")
	public FetchMemberEntity(String subject, EventStreamEntity collection, String versionOf, LocalDateTime timestamp, String transactionId, boolean isInEventSource, Model model) {
	...
	public FetchMemberEntity(long id) {
		this.id = id;
	}

	protected FetchMemberEntity() {
	}
