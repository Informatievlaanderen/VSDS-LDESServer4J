...
class MemberEntityListenerTest {
    ...
    void test_MemberHasNoIndex() {
        MemberEntity ldesMemberEntity = new MemberEntity("id", "collectionName", "versionOf", LocalDateTime.now(), null, true, "txId", "model");
    ...
    void test_MemberHasIndex() {
        MemberEntity ldesMemberEntity = new MemberEntity("id", "collectionName", "versionOf", LocalDateTime.now(), 23L, true, "txId", "model");
        BeforeConvertEvent<MemberEntity> beforeConvertEvent = new BeforeConvertEvent<>(ldesMemberEntity,
                "collection");
