...
public class EventStreamUpdaterChange {
    ...
    public void changeSet() {
        mongoTemplate.stream(new Query(), EventStreamEntityV1.class).forEach(eventStreamEntityV1 -> {
            ...
            mongoTemplate.remove(new Query(Criteria.where("_id").is(eventStreamEntityV1.getId())), EventStreamEntityV1.class);
        });
    ...
    public void rollback() {
        mongoTemplate.stream(new Query(), EventStreamEntityV2.class).forEach(eventStreamEntityV2 -> {
            ...
            mongoTemplate.remove(new Query(Criteria.where("_id").is(eventStreamEntityV2.getId())), EventStreamEntityV2.class);
        });
